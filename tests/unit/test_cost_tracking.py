...
(app/services/metrics_service.py). Scrap cost/rate were dropped from this app
entirely (docs/PROJECT_SPEC_PHASE4.md "Scrap removal") - only rework cost remains,
though classify_case_cost_bucket still recognizes a "scrap" bucket for backward
compatibility with the Scrap disposition/status (PROJECT_SPEC.md section 3.2)."""
...
from app.services.metrics_service import (
    ...
    compute_kpis,
    defect_case_derived_rework_count,
    sum_internal_rework_cost,
)


def test_sum_internal_rework_cost_uses_each_rows_own_rate():
    entries = [
        (5, decimal.Decimal("35.00")),  # reworked=5, rate=$35
        (3, decimal.Decimal("50.00")),  # a later day at the new $50 rate
    ]
    rework_cost = sum_internal_rework_cost(entries, fallback_rate=35.0)
    assert rework_cost == 5 * 35.0 + 3 * 50.0


def test_sum_internal_rework_cost_falls_back_for_rows_with_no_snapshot():
    """A row saved before Phase 4 existed has cost_per_drawer_at_time = None."""
    entries = [(4, None)]
    rework_cost = sum_internal_rework_cost(entries, fallback_rate=40.0)
    assert rework_cost == 4 * 40.0


def test_sum_internal_rework_cost_empty_entries_is_zero():
    assert sum_internal_rework_cost([], fallback_rate=35.0) == 0.0
...
def test_compute_kpis_total_internal_quality_cost_and_per_drawer():
    ...
    assert kpis["quality_cost_per_drawer_inspected"] == 1.75  # 175 / 100
...
def test_classify_case_cost_bucket_uses_status_when_closed():
    ...
    # the final physical outcome (scrap) wins over the stale disposition. (Only
    # "rework" feeds any cost calculation now, but the classification itself is
    # kept accurate for backward compatibility.)
    assert classify_case_cost_bucket("Closed - Scrapped", "Rework") == "scrap"


def test_defect_case_derived_rework_count_counts_one_drawer_per_case():
    today = dt.date(2026, 7, 24)
    cases = [
        (today, "Closed - Repaired", "Rework"),
        (today, "Closed - Scrapped", "Scrap"),  # scrap no longer counted
        (today, "Open", None),  # not dispositioned yet - counts toward neither
        (today, "Waiting", "Hold"),  # Hold is neither rework nor scrap
    ]
    assert defect_case_derived_rework_count(cases) == 1


def test_compute_internal_quality_cost_pure_daily_summary():
    result = compute_internal_quality_cost(
        daily_summary_entries=[(5, decimal.Decimal("35.00"))],
        fallback_case_rework_count=0,
        fallback_rate=35.0,
...
def test_compute_internal_quality_cost_falls_back_to_defect_cases_when_no_summary():
    """The bug being fixed: defect cases with a Rework disposition but no Daily
    Production Summary for that period must never show $0 cost."""
    result = compute_internal_quality_cost(
        daily_summary_entries=[],
        fallback_case_rework_count=3,
        fallback_rate=35.0,
